...
public class ToTariffConverter {
    ...
    private final static Repository REPOSITORY = Factory.INSTANCE.getRepository();
    ...
    public Tariff convertToTariff(String string){
        ...
        Optional<TariffExtension> tempTariffExtension = Optional.empty();

        for (int i = 2; i < fields.length; i++) {

            String[] param = fields[i].split(StringConstant.PARAMETER_DELIMITER.getValue());
            tempName = param[0];
            tempParameter = param[1];

            switch (tempName.toUpperCase()){
                case "PHONE":{
                    tempTariffExtension = REPOSITORY.findPhoneExtensionByName(tempName);
                    break;
                }
            }
